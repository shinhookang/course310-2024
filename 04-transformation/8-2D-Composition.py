...
mat2 S = mat2(2., 0., 0., 1.);  // 2x2 scaling matrix
float deg = 45;
float rad = radians(deg);
mat2 R = mat2(cos(rad), sin(rad), -sin(rad), cos(rad));  // 2x2 rotation matrix (10 degree)
void main()
{
    gl_Position = vec4(vertexPosition, 1.0); // w
    // The order of multiplication of the two matrices matters!
    // gl_Position.xy = S * R * vertexPosition.xy;  
    gl_Position.xy = R * S * vertexPosition.xy;  
